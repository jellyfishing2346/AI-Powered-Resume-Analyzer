...
# AI-Powered Resume Analyzer Deployment Script
...
if [ ! -f "test_api_clean.py" ]; then
    print_error "test_api_clean.py not found. Please run this script from the project root directory."
    exit 1
fi
...
python3 -c "from database import db_manager; db_manager.init_database(); print('Database initialized successfully')"

# Kill any existing processes
print_status "Stopping any existing servers..."
pkill -f "test_api_clean.py" 2>/dev/null || true
pkill -f "uvicorn.*test_api_clean" 2>/dev/null || true
...
nohup python3 test_api_clean.py > api_server.log 2>&1 &
API_PID=$!
...
pkill -f "test_api_clean.py" 2>/dev/null && echo "API server stopped"
...
ps aux | grep -E "(test_api_clean|npm start|react-scripts)" | grep -v grep | while read line; do
    echo "  $line"
done
EOF
...
## Files
- Main API: test_api_clean.py
- Database: database.py
- Frontend: frontend/
- Skills DB: skills.txt
